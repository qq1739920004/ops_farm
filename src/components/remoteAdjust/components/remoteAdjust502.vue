  ...
  >
    ...
    <div class="mainContent">
      ...
      >
        <el-row>
          <el-col
            v-if="basicTitleData"
            :span="12"
            v-for="(value, key, index) in basicTitleData"
            :key="index"
          >
            <el-form-item class="item" :label="value.name" :prop="key">
              <el-input
                style="width: 280px; height: 32px"
                v-model="basicParamsData[key]"
              />
            </el-form-item>
          </el-col>
          <el-col v-else :span="24" align="center">
        ...
        <div class="buttonarea">
          <el-button v-if="CalibTitleData" type="primary" @click="updatebasicParams">{{
            $t("work.submit")
          }}</el-button>
        </div>
      </el-form>
      <el-form
        ...
        ref="advanceFormRef"
        v-show="activeName == '4'"
        :rules="advance1ParamRules"
        :inline="true"
        :label-position="labelPosition"
        label-width="160px"
        :model="advanced1ParamsData"
        style="max-width: 1012px; margin-bottom: 20px"
      >
        <el-row>
          <el-col
            v-if="advance1TitleData"
            :span="12"
            v-for="(value, key, index) in advance1TitleData"
            :key="index"
          >
            <el-form-item class="item" :label="value.name" :prop="key">
              <el-input
                style="width: 280px; height: 32px"
                v-model="advanced1ParamsData[key]"
              />
            </el-form-item>
          </el-col>
          <el-col v-else :span="24" align="center">
...
const changeLogStatus = async (val: string, val2: any) => {
  if (switchStatus) {
    ...
    try {
      ...
      ElMessage({ type: "success", message: t('work.modificationSuccess'), duration: 1000 });
    } catch {
...
const getBasic = async () => {
  ...
  res.data.paramJson
    ? Object.assign(basicParamsData, JSON.parse(res.data.paramJson))
    : "";
};
// 高级参数1对应的值
...
const getProductList = async () => {
  ...
  GetcarProductpackage_API({
    ...
    .then((res: GetcarProductpackageResponseData) => {
      productList.value = res.data;
    })
    .catch(() => {
      productList.value = [];
    ...
    if (typeof objItem.range === "string") {
      ...
      let checkInt = (_rule: any, value: any, callback: any) => {
        ...
        if (value < min || value > max) {
          return callback(new Error(`${t('work.range')} ${min} 到 ${max} `));
        }
        callback();
      };
