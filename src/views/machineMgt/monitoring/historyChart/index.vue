  ...
  <div class="app_container">
    <div class="historyDriveContainer app_card">
      <div class="dateContainer">
        <span style="width:200px"> SN: {{ route.query.sn }} </span>
        <el-date-picker
          style="height: 34px; width: 200px"
          v-model="filterDate"
          type="date"
          :placeholder="t('chart.chooseDate')"
          :disabled-date="disabledDate"
          :clearable="false"
          @change="filterDateChange"
        />
        <el-time-picker
          style="height: 34px; width: 240px; margin-left: 10px; margin-right: 10px"
          v-model="timeRange"
          is-range
          :clearable="false"
          range-separator="-"
          :placeholder="t('chart.chooseTime')"
          format="HH:mm"
          @change="refreshData"
        />
        <el-tooltip
...
const initHDiffChart = () => {
  ...
  history_hDiffChart = echarts.init(document.getElementById("history_hDiffChart"));
  let option = {
    grid: {
      top: 30,
      bottom: 20,
...
const initSpeedChart = () => {
  ...
  history_speedChart.value = echarts.init(document.getElementById("history_speedChart"));
  let option = {
    grid: {
      top: 30,
...
const initAzimuthChart = () => {
  ...
  history_azimuthChart = echarts.init(document.getElementById("history_azimuthChart"));
  let option = {
    grid: {
      top: 30,
      left: 40,
...
const refreshData = async () => {
  const res = await driveTendency_API({
    sn: route.query.sn as string,
    st: formartDate(startTime.value),
    et: formartDate(endTime.value),
  });
  ...
  allData.forEach((item, index) => {
    if (index == 0) {
      ...
      speedData.push(0);
      azimuthData.push(0);
      if (formatHour(item.gnssTime) != formatHour(startTime.value)) {
      ...
      if (formatHour(item.gnssTime) != formatHour(endTime)) {
        hDiffXLabel.push(formatHour(item.gnssTime));
        // hDiffData.push(item.xOffset);
        hDiffData.push(item.xoffset * 100);
        speedData.push(item.speed);
