...
watch(
  () => props.monitorData.totalDevice,
  (newValue) => {
    animationTop(totalDeviceValue, newValue);
  }
);
watch(
  () => props.monitorData.activeDevice,
  (newValue) => {
    animationTop(activeDeviceValue, newValue);
  }
);
watch(
  () => props.monitorData.onlineDevice,
  (newValue) => {
    animationTop(onlineDeviceValue, newValue);
  }
);
watch(
  () => props.monitorData.addNowYearDevice,
  (newValue) => {
...
.top {
  ...
  .top_left {
    ...
    .top_tittle {
      ...
      h1 {
        text-shadow: 0px calc(2 / 937 * 100vh) calc(32 / 937 * 100vh)
          rgba(153, 255, 194, 0.8);
