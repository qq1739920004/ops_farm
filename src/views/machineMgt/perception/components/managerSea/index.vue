<template>
  <div class="container">
    ...
    <div class="screen" ref="screen">
      ...
      <div class="bottom">
        <div class="left">
          <Workarea class="workarea" :carAreas="carAreas" v-if="provinceCars?.length" />
...
import { getMonitorSea_API, getUserAuth } from "@/api/perception/index.ts";
import type { MonitorObj, recordsType } from "@/api/perception/type";
import socketAbroad from "@/store/socketAbroad";
...
const getMonitor = async () => {
  ...
  provinceCars.value = res.data.countryCars;
};
...
function handleSocketData(data: any) {
  ...
  if (data.type == "monitor") {
    ...
    provinceCars.value = data.data.countryCars;
...
:deep(.echarts-container) {
  transition: opacity 0.3s; /* 0.3秒的淡入淡出效果 */
}
.container {
  width: 100%;
  height: 100%;
