<template>
  <div class="box">
    <div class="title">
      ...
      <span>{{$t('perception.countryOnlineTotal')}}</span>
    </div>
...
let plist=props.carAreas.filter((item:any)=>item.countryEng)
const listy = plist.map((item: any) => item.onlineNum);
const listyTotal = plist.map((item: any) => item.totalNum);

const listx = plist.map((item: any) => item.countryEng);
// 获取图表节点
let bar = ref();
...
watch(props, (newValue) => {
  ...
  let plist=newValue.carAreas.filter((item:any)=>item.countryEng  )
  console.log(plist);
  option.yAxis.data = plist.map((item: any) => item.countryEng
);
