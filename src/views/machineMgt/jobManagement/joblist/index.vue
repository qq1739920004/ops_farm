      ...
      <div class="search_container">
        ...
        <div class="button_area">
          ...
          <el-button
            @click="
              router.push({
                path: '/machineMgt/jobManagement/taskManage',
                query: { companyId: pageInfo.companyId, carId: +pageInfo.carId },
              })
            "
            icon="MapLocation"
            class="btn2"
...
import {
  paddyWorkList_API,
  getCarDealerList_API,
  getPaddyWorkExport_API,
} from "@/api/jobManagement/index";
...
const changeBlur1 = () => {
  getDealerCarList();
  pageInfo.carId = t("work.pleaseSelect");
  paddyWorkList.value = [];
};
const changeBlur2 = () => {
  getPaddyWorkList();
...
const openExportDia = () => {
  const sns: any = CarDealerList.value.find((item: any, _idx) => {
    return item.id == pageInfo.carId;
  });
