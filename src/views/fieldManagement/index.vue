    ...
    <div class="info_box" v-show="infoShow">
      <div class="top_line">
        ...
        <div>
          ...
          <el-button type="danger" size="small" @click="deleteFields(fieldInfo.id)">
            {{ t("work.delete") }}
          </el-button>
        </div>
      </div>
      <div class="info_line">
        <el-row>
          <el-col :span="10"> {{ t("work.allArea") + "(m²)" }}:</el-col>
          <el-col :span="14"> {{ fieldInfo.area }}</el-col>
        </el-row>
        <el-row>
          <el-col :span="10"> {{ t("work.circumference") + "(m)" }}:</el-col>
          <el-col :span="14"> {{ fieldInfo.perimeter }}</el-col>
        </el-row>
        <el-row>
          <el-col :span="10"> {{ t("work.creationTime") }}:</el-col>
          <el-col :span="14"> {{ fieldInfo.createTime }}</el-col>
        </el-row>
        <el-row>
          <el-col :span="10"> {{ t("work.creator") }}:</el-col>
          <el-col :span="14"> {{ fieldInfo.createTime }}</el-col>
        </el-row>
        <el-row>
          <el-col :span="10"> {{ t("work.modifyTime") }}:</el-col>
          <el-col :span="14"> {{ fieldInfo.modifyTime }}</el-col>
        </el-row>
        <el-row>
          <el-col :span="10"> {{ t("work.modifier") }}:</el-col>
          <el-col :span="14"> {{ fieldInfo.modifier }}</el-col>
        </el-row>
        <el-row>
          <el-col :span="10"> {{ t("work.describe") }}:</el-col>
          <el-col :span="14"> {{ fieldInfo.description }}</el-col>
        </el-row>
        <el-row>
          <el-col :span="10"> {{ t("work.address") }}:</el-col>
          <el-col :span="14">
            {{ fieldInfo.address }}
            <el-button type="primary" size="small" @click="checkShare(fieldInfo.id)">{{
              t("work.shareDevice")
            }}</el-button>
          </el-col>
        </el-row>
...
const getFieldData = async () => {
  ...
  if (fieldList.value.length >= data.total) {
    ...
    const boundaries = data.records.map((item: any) => {
      return {
        id: item.id,
        boundaries: item.boundaries,
      };
    });
    const referenceLines = data.records.map((item: any) => {
      return {
        id: item.id,
        referenceLines: item.referenceLines,
      };
    });
    const obstacles = data.records.map((item: any) => {
      return {
        id: item.id,
