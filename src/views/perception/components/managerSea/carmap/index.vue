...
import { getUserAuth, getGeo_API } from "@/api/perception/index.ts";
// 导入Vue相关的库
...
//   // jsonData.value = allData;
//   const res = await getUserAuth();
//   if (jsonData.value[res.data.countryCode]) {
//     initMapChart(jsonData.value[res.data.countryCode]);
//   } else {
//     initMapChart(world);
//   }
...
//   getMapData();
// };
//  jsonData.value = await loadAllJsonData();
...
const initMapChart = (val: any) => {
  let goeList = getGeoList(val);
  mapOption = {
    animationDuration: 0,
...
const getGeoList = (val: any) => {
  ...
  goeList.push(geo);
  echarts.registerMap("world", val as any);
  return goeList;
};
