...
let yAxisData = dataList.map(r => r.typeName)
let xAxisData = dataList.map(r => r.totalCount)
let seriesData = dataList.map(r => r.onlineCount)
let seriesDataO=seriesData.map(function (item) {
                    return {
                        value: item,
                        symbol: barimg,
                    };
...
const option = {
    ...
    series: [
        ...
        //     label: false,//是否显示标签
        //     symbolSize: 17,//标记大小
        //     symbolOffset: (value:any,index:any)=>{
        //       if(seriesData[index.dataIndex]<100){
        //         return [100, 0]
        //       }else{
        //         console.log(value);
        //         return [-7, 0]
...
watch(props, (newValue) => {
    ...
    option.series[0].data = dataList.map(r => r.onlineCount).map(function (item) {
                    return {
                        value: item,
                        symbol: barimg,
                    };
