  ...
  <div class="SinoMap_component">
    ...
    <el-button-group class="btm_plo">
      ...
      <el-button
        ...
        @click="drawPolygons"
        v-if="!isEdit"
        :disabled="isDraw || pointsArray.length !== 0"
...
function openRealTimeChart_markerPopup(arg: any) {
  ...
  detailPolygonData.value.forEach((item: any) => {
    if (item) {
      tureFileds.push(item);
    }
  });
  emit("boundries", JSON.stringify([...tureFileds, ...turePoint]));
  area.value = calculatePolygonArea([...tureFileds, ...turePoint]);
  length.value = calculatePolygonPerimeter([...tureFileds, ...turePoint]);
}
function deleteLine(arg: any, index: any) {
  const markerToDelete = map._layers[arg];
  if (markerToDelete) {
    map.removeLayer(markerToDelete);
  }
  detailLineData.value[index] = null;
  let tureLine: any = [];
  detailLineData.value.forEach((item: any) => {
    if (item) {
      tureLine.push(item);
    }
  });
  emit("pickedPoints", JSON.stringify([...tureLine, ...pickedPointsArray.value]));
}
function deleteArea(arg: any, index: any) {
  const markerToDelete = map._layers[arg];
  if (markerToDelete) {
    map.removeLayer(markerToDelete);
  }
  detailPolygonData.value[index] = null;
  let tureFiled: any = [];
  detailPolygonData.value.forEach((item: any) => {
    if (item) {
      tureFiled.push(item);
  ...
  emit("boundries", JSON.stringify([...tureFiled, ...turePoint]));
}
...
function createMarker(list: any) {
  ...
  list.forEach((item: any) => {
    item.forEach((marker: any) => {
      if (marker.obstacles) {
        let lat: any = gcoordLngLat(
          marker.obstacles.split(",")[1],
          marker.obstacles.split(",")[0]
        );
...
watch(
  ...
  () => {
    ...
    emit("pickedPoints", JSON.stringify([...turePoint, ...pickedPointsArray.value]));
  },
  { deep: true }
);
//清除多边形图层组
...
// 已经存在的田块
const detailPolygonData = ref<any>([]);
const detailLineData = ref<any>([]);
// 创建多面形
...
let ABlineArray: any = reactive([]);
// 创建线
function createLine(list: any) {
  const lineDdata: any = [];
  list.forEach((it: any) => {
    it.forEach((item: any, index: any) => {
      if (item.referenceLines) {
        lineDdata.push(item.referenceLines);
        if (item.referenceLines.s72 === 0) {
          let arr = [
            gcoordLngLat(
              item.referenceLines.s94.s73[0].split(",")[1],
              item.referenceLines.s94.s73[0].split(",")[0]
            ),
            gcoordLngLat(
              item.referenceLines.s94.s73[1].split(",")[1],
              item.referenceLines.s94.s73[1].split(",")[0]
            ),
          ];
  ...
  detailLineData.value = lineDdata;
}
