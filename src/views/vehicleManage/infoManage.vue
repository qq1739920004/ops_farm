      ...
      <div class="table_container">
        ...
        >
          ...
          <el-table-column
            ...
            width=""
          />
          <el-table-column prop="ratedVoltage" label="额定电压" />
          <el-table-column
            prop="exeStandard"
            label="执行标准"
            show-overflow-tooltip
            width=""
          />
          <el-table-column prop="lineAccuracy" label="直线精度( ±cm )" />
          <el-table-column prop="produceNum" label="生产编号" />
          <el-table-column
            prop="produceTime"
            show-overflow-tooltip
            label="生产日期"
          />
          <el-table-column prop="ratedPower" label="额定功率" />
        ...
        >
          ...
          >
            ...
            <el-form-item label="生产日期" prop="produceTime">
              <el-date-picker
                v-model="formDataEdit.produceTime"
                type="datetime"
                value-format="yyyy-MM-dd HH:mm:ss"
                placeholder="选择日期"
                style="width: 100%"
          ...
          <el-row>
            <el-col :span="6" :offset="6">
              <el-button type="" size="small" @click="dialogVisibleEdit = false"
                >取消</el-button
              >
            </el-col>
            <el-col :span="6">
          ...
          if (res.data.page.list) {
            ...
            res.data.page.list.forEach((element) => {
              element.produceTime = element.produceTime
                ? this.dateTimeTrans(element.produceTime)
                : "/";
            });
        ...
        if (valid) {
          sacredCowQrSave_path(this.formData).then((res) => {
            if (res.data.page === 1) {
              ...
              res.data.page === 2
                ? this.$message.error("新建失败，出厂编号已存在")
                : "";
              res.data.page === 3
                ? this.$message.error("新建失败，出厂编号格式错误")
                : "";
            }
          });
        } else {
